...
// Basic Auth middleware - מגן על כל האתר
const basicAuth = (req, res, next) => {
  // דלג על callback (GitHub Actions צריך גישה)
  if (req.path.startsWith('/api/callback/')) return next();

  // אם אין הגדרת AUTH — אין סיסמה
  if (!AUTH_USER || !AUTH_PASS) return next();

  const authHeader = req.headers['authorization'];
  if (!authHeader || !authHeader.startsWith('Basic ')) {
    res.setHeader('WWW-Authenticate', 'Basic realm="APK Builder"');
...
app.post('/api/build', upload.single('file'), async (req, res) => {
  ...
  const orchestrator = new BuildOrchestrator({
    buildId, zipPath: req.file.path, originalName, buildType, versionName, versionCode, outputDir: OUTPUT_DIR,
