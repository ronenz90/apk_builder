...
const TRANSLATIONS = {
  en: {
    ...
    packageName: "Package Name", packageNamePlaceholder: "e.g. com.company.app (AAB only)",
  ...
  he: {
    ...
    packageName: "שם חבילה", packageNamePlaceholder: "למשל com.company.app (עבור AAB בלבד)",
...
export default function App() {
  ...
  const handlePackageNameChange = (val) => { setPackageName(val); packageNameRef.current = val; };
  const handleVersionNameChange = (val) => { setVersionName(val); versionNameRef.current = val; };
  const handleVersionCodeChange = (val) => { setVersionCode(val); versionCodeRef.current = val; };
  ...
  const handleResult = useCallback((msg) => {
    if (msg.type === 'success') {
      ...
      stopPolling();
      // עצור background service (הוא כבר שלח התראה)
      if (window.AndroidBridge) window.AndroidBridge.stopBuildPolling();
    }
    if (msg.type === 'error') {
  ...
  const onDrop = useCallback(async (accepted, rejected) => {
    ...
    formData.append("versionName", versionNameRef.current.trim());
    formData.append("versionCode", versionCodeRef.current.trim());
    formData.append("packageName", packageNameRef.current.trim());
    ...
    xhr.onload = () => {
      ...
      if (xhr.status === 200) {
        ...
        // הפעל background service לקבלת התראות גם ברקע
        if (window.AndroidBridge) {
          window.AndroidBridge.startBuildPolling(data.buildId);
        }
            ...
            <div className="version-fields">
              <div className="version-field">
                ...
                <input type="text" placeholder={t.versionNamePlaceholder}
                  value={versionName} onChange={e => handleVersionNameChange(e.target.value)}
                  className="version-input" />
              </div>
              <div className="version-field">
                <label>{t.versionCode}</label>
                <input type="number" placeholder={t.versionCodePlaceholder}
                  value={versionCode} onChange={e => handleVersionCodeChange(e.target.value)}
                  className="version-input" min="1" />
              </div>
              {buildType === "aab" && (
                <div className="version-field" style={{gridColumn: "1 / -1"}}>
                  <label>{t.packageName}</label>
                  <input type="text" placeholder={t.packageNamePlaceholder}
                    value={packageName} onChange={e => handlePackageNameChange(e.target.value)}
                    className="version-input" />
                </div>
              )}
            </div>
