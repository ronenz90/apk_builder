...
function getAuthToken() {
  try { return btoa(`${window._u||''}:${window._p||''}`); } catch { return ''; }
}
...
export default function App() {
  ...
  const connectWS = useCallback((buildId) => {
    const ws = new WebSocket(`${WS_BASE}/ws/${buildId}?token=${getAuthToken()}`);
    wsRef.current = ws;
    ws.onmessage = (e) => {
  ...
  const onDrop = useCallback(async (accepted, rejected) => {
    ...
    xhr.onload = () => {
      ...
      if (xhr.status === 200) {
        ...
        if (versionName) addLog(`Version: ${versionName} (${versionCode || 'code from ZIP'})`);
      } else { setStage("error"); setErrorMsg(data.error || "Upload failed"); }
    };
      ...
      <main className="main">
        ...
        {stage === "done" && (
          <div className="result-card success">
            ...
            <a href={`${API_URL}${apkUrl}`} download className="download-btn">↓ {t.download} {fileExt}</a>
            {!isAAB && qrCode && (
